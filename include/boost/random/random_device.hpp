...
/**
 * Class \random_device models a non-deterministic UniformRandomNumberGenerator.
 ...
 * [endsect]
 */
class random_device : public noncopyable, public detail::random_provider
{
public:
    //! \note    required by the UniformRandomNumberGenerator concept.
    ...
    //!          operator(), in the range min() to log2(max()+1).
    //! \note    A deterministic random number generator (e.g. a pseudo-random
    ...
    //! \brief   Allow \random_device to fulfill the SeedSeq concept
    //!          and therefore be compatible as a PseudoRandomNumberGenerator
    //!          seed() argument.
    //! \param   first  the first of the sequence requested
    //! \param   last   the last of the sequence requested
    //! \throws  entropy_error
    //! \example
