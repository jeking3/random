...
public:
      ...
      : hProv_(NULL)
    {
        ...
        if (!boost::detail::winapi::CryptAcquireContextW(
            ...
            NULL,
            NULL,
            boost::detail::winapi::PROV_RNG_,
            boost::detail::winapi::CRYPT_VERIFYCONTEXT_ | boost::detail::winapi::CRYPT_SILENT_))
        {
            BOOST_THROW_EXCEPTION(system::system_error(
                boost::detail::winapi::GetLastError(), system::system_category(), "CryptAcquireContextW"));
        }
#else
        // With ANSI APIs, the behavior is backwards compatible to previous releases
        boost::detail::winapi::CHAR_ buffer[256];
        boost::detail::winapi::DWORD_ type;
        boost::detail::winapi::DWORD_ len;
        std::string provider = token.empty() ? "Microsoft Base Cryptographic Provider v1.0" : token;

        // Find the type of a specific provider
        for (boost::detail::winapi::DWORD_ i = 0; ; ++i)
        {
            len = sizeof(buffer);
            if (!boost::detail::winapi::CryptEnumProvidersA(i, NULL, 0, &type, buffer, &len))
            {
                BOOST_THROW_EXCEPTION(system::system_error(
                    boost::detail::winapi::GetLastError(), system::system_category(), "CryptEnumProvidersA"));
            }
            if (buffer == provider) {
                break;
            }
        ...
        {
            BOOST_THROW_EXCEPTION(system::system_error(
                boost::detail::winapi::GetLastError(), system::system_category(), "CryptAcquireContextA"));
        }
#endif
    }
    ...
    ~random_device_wincrypt() BOOST_NOEXCEPT
    {
        if (hProv_)
        {
            ignore_unused(boost::detail::winapi::CryptReleaseContext(hProv_, 0));
        }
    }
    ...
    {
        result_type result;
        if (!boost::detail::winapi::CryptGenRandom(hProv_, sizeof(result), boost::detail::winapi::detail::cast_ptr(&result)))
        {
            BOOST_THROW_EXCEPTION(system::system_error(
                boost::detail::winapi::GetLastError(), system::system_category(), "CryptGenRandom"));
        }
        return result;
    }

private:
    boost::detail::winapi::HCRYPTPROV_ hProv_;
};
